...
static void main_window_load(Window *window) {
  ...
  s_hour_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_SIPLE_BOLD_47));
  s_minute_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_SIPLE_LITE_47));
  
  //Set Hour Layer
    s_hour_layer = text_layer_create(
    GRect(bounds.origin.x, (bounds.size.h - 92) / 2, bounds.size.w, text_height));
